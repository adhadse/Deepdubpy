...
class DeepdubSentences:
  def __init__(self, file, slice_from=None, slice_to=None):
  ...
  def get_sentences(self, hashed=True):
    ...
    if hashed:
      # Set index as the hash of starting and ending time.
      sentence_df.set_index(["hash"], inplace=True)
    else:
      # Otherwise set "start" and "end" and index.
      sentence_df.set_index(["start", "end"], inplace=True)

    return sentence_df
  
  def save_subs(path_to_file):
